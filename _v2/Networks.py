...
from lossFN import RMSELoss
from lossFN import CustomLoss

class FCNet(nn.Module):
    def __init__(self, model_name, architecture_name=None, weight_decay=0, dropout_rate=0, penalty=None):
        ...
        self.patience = None
        # architecture
        if architecture_name == 'fcbasic':
            from Architectures import FCBasic
            self.architecture = FCBasic(dropout_rate)
        else:
    ...
    def forward(self, context):
        context = self.architecture(context)
        return context
    
    def train_epoch(self, trainloader, device):
        self.train()
        cost, s_cost= 0, 0
        for batch_context, batch_labels in trainloader: # loop iterations correspond to batches
            context, y = batch_context.to(device), batch_labels.to(device)
            # prediction error
            preds = self(context)
    ...
    def test_epoch(self, testloader, device):
        ...
        with torch.no_grad(): # disable gradient calculation
            for batch_context, batch_labels in testloader:
                context, y = batch_context.to(device), batch_labels.to(device)
                preds = self(context)
                loss = self.loss_fn(preds, y)
                s_loss = self.s_loss_fn(preds, y)
    ...
    def log_epoch(self, header, runpath, nr_epochs):
        ...
        if len(self.s_testcost) >= 1:
            if self.s_testcost[-1] == min(self.s_testcost[0:]):
                self.patience = 4
                torch.save(self.state_dict(), runpath + self.model_name + '.pth')
...
class ImNet(nn.Module):
    def __init__(self, model_name, architecture_name=None, weight_decay=0, penalty=None):
        ...
        self.patience = None
        # architecture
        if architecture_name == 'im':
            from Architectures import Im
            self.architecture = Im()
        else:
            print(f'architecture {architecture_name} is not defined')
            1/0
        self.optimizer = torch.optim.Adam(self.parameters(), weight_decay=weight_decay)
