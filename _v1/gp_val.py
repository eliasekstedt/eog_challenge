...
def objective(param):
    ...
    model.train_model(trainloader=loader0, testloader=loader1, nr_epochs=hparam['nr_epochs'], runpath=runpath, device=device)
    ...
    from util.Readers import Res18FCReader as Reader
    from util.Evaluation_tools import Evaluator
    model.eval()
    evaluator = Evaluator(runpath, model, Reader, path, hparam, device)
    print(f'evaldata\n{evaluator.evaldata}')
