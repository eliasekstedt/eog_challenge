...
class JustResNet(nn.Module):
    def __init__(self, architecture_name=None, weight_decay=0, dropout_rate=0):
        super(JustResNet, self).__init__()
        self.loss_fn = nn.MSELoss() # try the RMSE in util.Special later
        self.traincost, self.testcost = [], []
        # architecture
        if architecture_name == 'justres':
            from util.Architectures import JustRes
            self.architecture = JustRes()
        else:
    ...
    def forward(self, x, context):
        x, rescomponent = self.architecture(x, context)
    ...
    def train_epoch(self, trainloader, device):
        ...
        for batch_images, batch_context, batch_labels, _ in trainloader: # loop iterations correspond to batches
            ...
            # prediction error
            preds, rescomponent = self(x, context)
...
class JustFCNet(nn.Module):
    def __init__(self, architecture_name=None, weight_decay=0, dropout_rate=0):
        super(JustFCNet, self).__init__()
        self.loss_fn = nn.MSELoss() # try the RMSE in util.Special later
        self.traincost, self.testcost = [], []
        # architecture
        if architecture_name == 'justfc':
            from util.Architectures import JustFC
            self.architecture = JustFC()
        else:
            print('architecture not defined')
            1/0
        self.optimizer = torch.optim.Adam(self.parameters(), weight_decay=weight_decay)
    ...
    def train_epoch(self, trainloader, device):
        self.train()
        cost = 0
        for batch_features, batch_labels in trainloader: # loop iterations correspond to batches
            features, labels = batch_features.to(device), batch_labels.to(device)
            # prediction error
            preds = self(features)
            loss = torch.sqrt(self.loss_fn(preds, labels))
            # Backpropagation
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()
